# Web App Dashboard — Plan de Implementación

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Crear una Web App pública en Google Apps Script que lea el Sheet de WhatsApp y muestre un dashboard oscuro profesional con KPIs, urgencias Alta y barras de categorías/palabras clave.

**Architecture:** Un solo archivo `WebApp.gs` con tres funciones: `calcularMetricas(datos)` procesa los datos del Sheet, `generarHTML(metricas)` construye el HTML completo con estilos inline, y `doGet()` orquesta ambas y retorna `HtmlService`. Sin librerías externas. El deployment es manual desde Apps Script.

**Tech Stack:** Google Apps Script (ES5), HtmlService, SpreadsheetApp, Utilities.formatDate, Session.getScriptTimeZone.
...
herramientas/clasificador-whatsapp/
  WebApp.gs    ← NUEVO: calcularMetricas(), generarHTML(), doGet(), testWebApp()
```

---

## Task 1: `WebApp.gs` — función `calcularMetricas()`

**Files:**
- Create: `herramientas/clasificador-whatsapp/WebApp.gs`

- [ ] **Paso 1.1: Crear el archivo con `calcularMetricas()` y su test**

Crear `herramientas/clasificador-whatsapp/WebApp.gs` con este contenido:
...
```bash
git add herramientas/clasificador-whatsapp/WebApp.gs
git commit -m "feat: agregar calcularMetricas() a WebApp.gs"
```

---

## Task 2: `WebApp.gs` — `generarHTML()`, `doGet()` y `testWebApp()`

**Files:**
- Modify: `herramientas/clasificador-whatsapp/WebApp.gs` (agregar al final)

- [ ] **Paso 2.1: Agregar las tres funciones al final de `WebApp.gs`**

Abrir `WebApp.gs` y **agregar al final** (después de `testCalcularMetricas`) este código:
...
```bash
git add herramientas/clasificador-whatsapp/WebApp.gs
git commit -m "feat: agregar generarHTML(), doGet() y testWebApp() a WebApp.gs"
```

---

## Task 3: Desplegar como Web App (manual)

> Tarea en el navegador — no requiere código.

- [ ] **Paso 3.1: Copiar `WebApp.gs` a Apps Script**

1. Abrir Apps Script del proyecto Clasificador WhatsApp Wellbive
2. Clic en **+** junto a Archivos → Script → nombre `WebApp`
3. Pegar el contenido completo de `herramientas/clasificador-whatsapp/WebApp.gs`
4. Guardar (Ctrl+S)

- [ ] **Paso 3.2: Ejecutar `testCalcularMetricas`**

1. Seleccionar función `testCalcularMetricas` en el selector
2. Ejecutar (▶)
3. Abrir **Ver > Registros** (Ctrl+Enter)
4. **Resultado esperado:** `testCalcularMetricas: OK — todas las verificaciones pasaron`

- [ ] **Paso 3.3: Ejecutar `testWebApp`**

1. Seleccionar función `testWebApp`
2. Ejecutar (▶)
3. **Resultado esperado en log:**
```
testWebApp: OK — HTML válido (XXXX chars)
Puedes abrir la Web App desde: Implementar > Administrar implementaciones
```

- [ ] **Paso 3.4: Desplegar como Web App pública**

1. En Apps Script: clic en **"Implementar"** (botón azul arriba a la derecha)
2. Seleccionar **"Nueva implementación"**
3. Clic en el ícono de engranaje ⚙️ → **"Aplicación web"**
4. Configurar:
   - Descripción: `Monitor WhatsApp Wellbive`
   - Ejecutar como: **Yo** (tu cuenta de Google)
   - Quién tiene acceso: **Cualquier usuario**
5. Clic en **"Implementar"**
6. Autorizar permisos (acceso a Sheets + enviar email)
7. **Copiar la URL** que aparece — esa es la URL pública del dashboard

- [ ] **Paso 3.5: Verificar en el navegador**

1. Abrir la URL copiada en el navegador
2. **Resultado esperado:** Dashboard oscuro con título "WELLBIVE · Monitor WhatsApp", 4 KPIs, tabla de Urgencias Alta, barras de categorías y palabras clave
...
**Cobertura del spec:**
- ✓ URL pública sin login — `doGet()` con acceso "Cualquier usuario"
- ✓ Estilo oscuro — fondo `#0d0d0d`, tarjetas `#111827`
- ✓ 4 KPIs: totalSemana, urgenciasAltaCount, topKeyword, topCategoria
- ✓ Tabla urgencias Alta con fecha/nombre/acción
- ✓ Barras CSS categorías (azul) y palabras clave (verde)
- ✓ "Esta semana" = lunes a hoy (consistente con Dashboard.gs)
- ✓ Responsive (grid auto-fit + media query a 640px)
- ✓ Sin datos → muestra zeros / mensajes "Sin datos" sin errores
- ✓ Timestamp de carga en header

**Consistencia de tipos:**
- `calcularMetricas(datos)` → objeto con exactamente los campos que usa `generarHTML(metricas)` ✓
- `urgenciasAlta[i].fecha`, `.nombre`, `.accion` — usados en `generarHTML` ✓
- `categorias[i].nombre`, `.count`, `.pct` — usados en `generarHTML` ✓
- `keywords[i].nombre`, `.count`, `.pct` — usados en `generarHTML` ✓
- `doGet()` llama `calcularMetricas()` y `generarHTML()` — ambas definidas en Task 1 y 2 ✓
- `testWebApp()` llama `doGet()` — definida en Task 2 ✓
